...
# Tabs erstellen
tabs = st.tabs(["Deine Statistiken", "Vergleich"])
...
STAT_LABELS = {
    ...
    "total_time_played": "Gesamtspielzeit (Sek.)",
    ...
    if steam_id:  # Daten laden
        ...
        if "error" in result:
            ...
            if not games:
                ...
                # Top 5 Spiele und Sonstige
                st.subheader("Deine Top 5 Spiele")
                top_5 = df_games.head(5)
                rest = df_games.iloc[5:]
                rest_sum = rest["Playtime (Hours)"].sum()
                other_row = pd.DataFrame([{"Name": "Sonstige", "Playtime (Hours)": rest_sum}])
                top_5_with_other = pd.concat([top_5, other_row], ignore_index=True)

                # Tortendiagramm erstellen
                fig = px.pie(
                    top_5_with_other,
                    names="Name",
                    values="Playtime (Hours)",
                    title="Top 5 Spiele und Sonstige",
                            ...
                            if len(filtered_stats) > 0:
                                ...
                                for stat_key in relevant_stats:
                                    if stat_key in stats_dict:
                                        ...
                                        # 1) Trefferquote direkt nach "Abgefeuerte Schüsse" ausgeben
                                        ...
                                        # 2) Siegesquote direkt nach "Gespielte Matches" ausgeben
        ...
        if "error" in result_1:
            ...
            if not games_1:
                ...
                if not common_games.empty:
                    ...
                    # Balkendiagramm für Spielzeiten
                    chart_data = common_games.rename(columns={
                        "Playtime (Hours)_Player1": "Spieler 1",
                        "Playtime (Hours)_Player2": "Spieler 2"
                    }).set_index("Name")[["Spieler 1", "Spieler 2"]]
