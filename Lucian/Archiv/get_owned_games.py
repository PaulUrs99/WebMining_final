...
def update_csv(steam_id, games_data, output_csv):
    ...
    """
    # Wenn der Nutzer keine Spiele besitzt, Dummy-Eintrag erstellen
    if not games_data:
        new_entries = [{
        ...
        # Normale Verarbeitung, wenn Spiele vorhanden sind
        new_entries = []
    ...
    # Positiv formulierte Bedingung: Wenn die Datei existiert, anhängen
    if os.path.exists(output_csv):
        # An bestehende Datei anhängen
        new_entries_df.to_csv(output_csv, mode='a', header=False, index=False)
    else:
        # Neue Datei erstellen und Spaltenüberschriften schreiben
        new_entries_df.to_csv(output_csv, index=False)

    # Steam-ID zur Menge der verarbeiteten IDs hinzufügen
    processed_ids.add(steam_id)
    print(f"länge der processed_ids: {len(processed_ids)}")
